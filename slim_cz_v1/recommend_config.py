...
def generate_custom_config(dataset_tokens: int, vocab_size: int, base_config: dict) -> dict:
    ...
    new_num_heads = min([h for h in possible_heads if new_d_model % h == 0], default=8)
...
def main():
    ...
    print(
        f"{'Config':<25} {'Params':<12} {'Optimal':<12} {'Ratio':<8} {'Pred.Loss':<11} {'Efficiency':<15} {'Quality'}"
    )
    print("-" * 110)
    ...
    print("  Compute-optimal (Chinchilla): 1 param : 20 tokens")
    print("  Small model optimal:          1 param : 100-150 tokens")
    ...
    if best_analysis["efficiency_score"] >= 70:
        ...
        print()
        print("This configuration is well-matched to your dataset:")
        print(
            f"  Model parameters:      {best_analysis['params']:,} ({best_analysis['params'] / 1e6:.2f}M)"
        )
        print(
            f"  Optimal tokens:        {best_analysis['optimal_tokens']:,} ({best_analysis['optimal_tokens'] / 1e6:.2f}M)"
        )
        print(f"  Your tokens:           {dataset_tokens:,} ({dataset_tokens / 1e6:.2f}M)")
        print(f"  Token ratio:           {best_analysis['token_ratio']:.2f}x optimal")
        print(
            f"  Efficiency score:      {Colors.GREEN}{best_analysis['efficiency_score']:.1f}/100{Colors.ENDC}"
        )
        ...
        print(
            f"  Issue: {'Too large for dataset' if best_analysis['token_ratio'] < 1 else 'Suboptimal'}"
        )
        print()
        ...
        custom_analysis = analyze_config(
            "custom_optimized", custom_config, dataset_tokens, vocab_size
        )

        print()
        print("Custom configuration generated:")
        print(
            f"  Model parameters:      {custom_analysis['params']:,} ({custom_analysis['params'] / 1e6:.2f}M)"
        )
        print(
            f"  Optimal tokens:        {custom_analysis['optimal_tokens']:,} ({custom_analysis['optimal_tokens'] / 1e6:.2f}M)"
        )
        print(f"  Token ratio:           {custom_analysis['token_ratio']:.2f}x optimal")
        print(
            f"  Efficiency score:      {Colors.GREEN}{custom_analysis['efficiency_score']:.1f}/100{Colors.ENDC}"
        )
    ...
    pred_loss = predict_loss(
        calculate_model_parameters(recommended_config), dataset_tokens, vocab_size
    )
    ...
    print()
    print("Loss progression (estimated):")
    for epoch in [1, 5, 10, 20, recommended_config["train"]["epochs"]]:
        if epoch <= recommended_config["train"]["epochs"]:
            ...
            estimated_loss = math.log(vocab_size) - (math.log(vocab_size) - pred_loss) * (
    ...
    print("  ✓ Val loss decreasing steadily")
    ...
    print("  ✗ Val loss not decreasing after 15 epochs")
    print("  ✗ Val loss increasing")
    print()
    print("If you see bad signs:")
    print("  1. Stop training (Ctrl+C)")
    print("  2. Try smaller model or more epochs")
    ...
    if current_checkpoint.exists():
        ...
        print("  Validation loss:  7.84 (POOR)")
        print("  Quality:          Generates only ⁇ symbols")
        print()
        ...
        print("  Quality:          Functional Czech text")
        print()
        print(
            f"{Colors.BOLD}Expected improvement: {((7.84 - pred_loss) / 7.84 * 100):.1f}% better{Colors.ENDC}"
        )
    ...
    print(
        f"Training time: ~{train_config.get('epochs', 30) * 2:.0f}-{train_config.get('epochs', 30) * 4:.0f} min (GPU)"
    )
    print()
    print(
        f"{Colors.CYAN}→ This configuration is mathematically optimized for your dataset!{Colors.ENDC}"
    )
    print()
