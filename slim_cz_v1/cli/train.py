...
""",
    ...
    # Required arguments
    parser.add_argument(
        "--config", "-c", type=Path, required=True, help="Path to YAML configuration file"
    )
    parser.add_argument(
        "--tokens", "-t", type=Path, required=True, help="Path to tokenized data file"
    )
    parser.add_argument(
        "--output", "-o", type=Path, required=True, help="Output directory for checkpoints and logs"
    )
    ...
    parser.add_argument(
        "--resume-from", type=Path, default=None, help="Resume from a specific checkpoint file path"
    )
...
def main():
    ...
    console.configure(verbose=args.verbose, quiet=args.quiet, color=not args.no_color)
    ...
    try:
        ...
        if final_result:
            console.success(
                f"Training completed! Best PPL: {final_result['best_val_perplexity']:.2f}"
            )
            sys.exit(0)
        else:
