    ...
    def set_resume_step(self, global_step: int):
        if self.enabled:
            ...
            self.writer = SummaryWriter(
                log_dir=str(self.log_dir / "tensorboard"), purge_step=global_step
            )
...
def load_tokenized_data(
    tokens_path: Path, seq_len: int, vocab_size: int, val_split: float = 0.1
    ...
    console.verbose(
        f"Train: {stats['train_sequences']:,} | Val: {stats['val_sequences']:,} sequences"
    )
...
def create_dataloaders(
    ...
    if (console.in_notebook or console.in_kaggle) and num_workers > 0:
        console.verbose("Notebook detected - setting num_workers=0")
        num_workers = 0
...
class Trainer:
    ...
    def __init__(
        self, config: dict, output_dir: Path, enable_tensorboard: bool = True, use_tqdm: bool = True
    ):
        """
        Initialize trainer.
    ...
    def build_model(self, vocab_size: int, seq_len: int) -> nn.Module:
        ...
        console.verbose(
            f"Building model: d_model={model_cfg.get('d_model', 256)}, "
    ...
    def train_epoch(
        ...
        if use_progress_bar:
            try:
                pbar = tqdm_auto(
                    train_loader, desc=f"Epoch {epoch}/{self.epochs}", leave=True, mininterval=1.0
                )
            except Exception:
                pbar = train_loader
        ...
        for batch_idx, (input_ids, labels) in enumerate(pbar):
            ...
            if use_progress_bar:
                pbar.set_postfix(
                    {"loss": f"{loss.item():.4f}", "ppl": f"{torch.exp(loss).item():.2f}"}
                )
            else:
                ...
                console.batch(
                    ...
                    num_batches,
                    loss.item(),
                    {"ppl": torch.exp(loss).item(), "lr": scheduler.get_last_lr()[0]},
                )
    ...
    def evaluate(
        self, model: nn.Module, val_loader: torch.utils.data.DataLoader, epoch: int = 0
    ) -> tuple[float, float]:
        ...
        if self.tb_logger.enabled and epoch > 0:
            self.tb_logger.writer.add_histogram(
                "Validation/Batch_Losses", torch.tensor(batch_losses), epoch
            )
    ...
    def save_checkpoint(
        ...
        if self.tb_logger.enabled:
            self.tb_logger.writer.add_text(
                "Checkpoints",
                f"Saved checkpoint at epoch {epoch} (global_step {global_step}), is_best={is_best}",
                global_step,
            )
        ...
        fd, tmp_path = tempfile.mkstemp(dir=str(self.output_dir), suffix=".pt.tmp")
        try:
            os.close(fd)
        ...
        if is_best:
            ...
            fd, tmp_best = tempfile.mkstemp(dir=str(self.output_dir), suffix=".pt.tmp")
            try:
                os.close(fd)
    ...
    def load_checkpoint(self, checkpoint_path: Path | None = None) -> dict[str, Any] | None:
        ...
        # nosec B614: weights_only=False required for full checkpoint (optimizer, scheduler, RNG states)
    ...
    def train(
        ...
        for epoch in range(start_epoch, self.epochs + 1):
            ...
            if checkpoint_enabled and (epoch % save_every == 0 or is_best):
                self.save_checkpoint(model, optimizer, scheduler, epoch, global_step, is_best)
        ...
        # Final
        self.save_checkpoint(model, optimizer, scheduler, self.epochs, global_step, is_best=False)
