...
from ..preprocessing.base import print_info, print_section, print_success, print_warning
...
class StatisticsCollector:
    ...
    def _calculate_vocabulary_coverage(self, word_counter: Counter) -> dict[str, int]:
        ...
        for _word, count in sorted_words:
            cumulative += count
            vocab_size += 1
    ...
    def analyze_tokenizer(
        ...
        compression_ratio = (
            corpus_stats.total_characters / corpus_stats.total_words
            if corpus_stats.total_words > 0
            else 0.0
        )
        ...
        byte_pair_efficiency = (
            1.0 - (bpe_vocab / (char_level_vocab * 100)) if char_level_vocab > 0 else 0.0
        )
    ...
    def print_statistics(
        self, corpus_stats: CorpusStatistics, tokenizer_stats: TokenizerStatistics | None = None
    ):
        """
        ...
        print(
            f"   Total bytes:         {corpus_stats.total_bytes:,} ({corpus_stats.corpus_size_mb:.2f} MB)"
        )
        ...
        print("      (unique_words / total_words)")
        print(f"   STTR:                {corpus_stats.sttr:.6f}")
        print("      (TTR on first 10k tokens)")
        print(f"   RTTR (Guiraud):      {corpus_stats.rttr:.4f}")
        print("      (unique_words / sqrt(total_words))")
        print(f"   MSTTR:               {corpus_stats.msttr:.6f}")
        print("      (Mean TTR over 1k-token segments)")
        ...
        if tokenizer_stats:
            ...
            print("      (1.0 = perfect, <1.0 = over-segmentation)")
            ...
            print("\n   === TOKEN STATISTICS ===")
            print(
                f"   Avg token length:    {tokenizer_stats.avg_token_length:.4f} ± {tokenizer_stats.token_length_std:.4f} chars"
            )

            if tokenizer_stats.sample_compression_rates:
                avg_compression = sum(tokenizer_stats.sample_compression_rates) / len(
                    tokenizer_stats.sample_compression_rates
                )
            ...
            print(
                f"      ({tokenizer_stats.czech_diacritic_preservation * 100:.2f}% of vocab contains diacritics)"
            )
